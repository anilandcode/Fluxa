...
init_registry();
gsap4.registerPlugin(ScrollTrigger);
var ScrollController = class {
  ...
  init() {
    ...
    gsap4.ticker.add((time) => {
      this.lenis?.raf(time * 1e3);
    });
    gsap4.ticker.lagSmoothing(0);
  }
...
var FluxaMotion = class {
    ...
    this.matchMedia = withAccessibility(
      (conditions) => {
        if (conditions.reduceMotion) {
          gsap4.defaults({ duration: 0 });
        } else {
          gsap4.defaults({ duration: this.preset.duration.normal });
        }
        if (!conditions.reduceMotion) {
  ...
  switchPreset(presetName, overrides) {
    ...
    gsap4.set(this.container.querySelectorAll("*"), { clearProps: "all" });
    this.introTimeline = this.timelineManager.createIntro(this.container);
    this.introTimeline.play();
    return this;
  }
...
function splitTextReveal(target, config = {}) {
  ...
  const el = typeof target === "string" ? document.querySelector(target) : target;
  if (!el) return { tween: gsap4.to({}, {}), elements: [], revert: () => {
  } };
  const originalHTML = el.innerHTML;
  const parts = splitText(el, type);
  if (rotationX !== 0) el.style.perspective = "600px";
  const tween = gsap4.from(parts, { autoAlpha: 0, y, rotationX, duration, ease, stagger: { amount: staggerAmount, from: "start" } });
...
function scaleHeading(targets, config = {}) {
  ...
  return gsap4.from(targets, { autoAlpha: 0, scale: fromScale, duration, ease, stagger });
}
function scrambleText(target, config = {}) {
  const { duration = 1.5, ease = "none", chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%" } = config;
  const el = typeof target === "string" ? document.querySelector(target) : target;
  if (!el) return { timeline: gsap4.timeline(), revert: () => {
  } };
...
function letterSpacingReveal(targets, config = {}) {
  ...
  return gsap4.fromTo(targets, { autoAlpha: 0, letterSpacing: fromSpacing }, { autoAlpha: 1, letterSpacing: toSpacing, duration, ease });
}
gsap4.registerPlugin(ScrollTrigger);
...
function horizontalScroll(container, content, config = {}) {
  ...
  const tween = gsap4.to(contentEl, {
    x: -scrollWidth,
    ease: "none",
    scrollTrigger: {
      trigger: container,
...
function scrollProgress(target, config = {}) {
  ...
  gsap4.set(target, { transformOrigin: direction === "horizontal" ? "left center" : "center top", [prop]: 0 });
  return gsap4.to(target, {
    [prop]: 1,
    ease: "none",
